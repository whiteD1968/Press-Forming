...
export function ResearchDetailPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (error || !data) {
        setMessage(error?.message ?? "Research source not found.");
        return;
      }
...
export function AtlasDetailPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (error || !data) {
        setMessage(error?.message ?? "Atlas entry not found.");
        return;
      }
...
export function MaterialDetailPage({ id }: { id: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (error || !data) {
        setMessage(error?.message ?? "Material not found.");
        return;
      }
...
export function ProductDetailPage({ id }: { id: string }) {
  ...
  const [media, setMedia] = useState<LibraryMedia[]>([]);
  const [message, setMessage] = useState("Loading...");
  useEffect(() => { async function load() { const supabase = createClient(); const { data, error } = await supabase.from("products").select("*, materials(id, name), vendors(id, name, website_url)").eq("id", id).single(); if (error || !data) { setMessage(error?.message ?? "Product not found."); return; } setItem(data as Product); setMedia((await loadMedia("product", [id])).get(id) ?? []); setMessage(""); } load(); }, [id]);
  if (!item) return <section className="page-shell narrow-shell"><div className="notice">{message}</div></section>;
...
export function EquipmentDetailPage({ id }: { id: string }) {
  ...
  const [media, setMedia] = useState<LibraryMedia[]>([]);
  const [message, setMessage] = useState("Loading...");
  useEffect(() => { async function load() { const supabase = createClient(); const { data, error } = await supabase.from("equipment").select("*").eq("id", id).single(); if (error || !data) { setMessage(error?.message ?? "Equipment not found."); return; } setItem(data as Equipment); setMedia((await loadMedia("equipment", [id])).get(id) ?? []); setMessage(""); } load(); }, [id]);
  if (!item) return <section className="page-shell narrow-shell"><div className="notice">{message}</div></section>;
...
export function ContributePage() {
  ...
  const [message, setMessage] = useState("Loading...");
  useEffect(() => { async function load() { const supabase = createClient(); const { data: auth } = await supabase.auth.getUser(); if (!auth.user) { setMessage("Sign in to contribute."); return; } const { data: profile } = await supabase.from("profiles").select("is_active").eq("id", auth.user.id).single(); if (!profile?.is_active) { setMessage("Inactive accounts cannot contribute."); return; } setAllowed(true); setMessage(""); } load(); }, []);
