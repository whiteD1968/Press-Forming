...
const adminConfigs = {
  research: { table: "research_sources", title: "Research", titleField: "title", newHref: "/contribute/research/new", editBase: "/contribute/research", openBase: "/research", select: "id, title, source_type, publication_year, status, is_published, updated_at, added_by" },
  materials: { table: "materials", title: "Materials", titleField: "name", newHref: "/contribute/materials/new", editBase: "/contribute/materials", openBase: "/materials", select: "id, name, material_family, alloy_grade, status, is_published, updated_at, created_by" },
  products: { table: "products", title: "Products", titleField: "product_name", newHref: "/contribute/products/new", editBase: "/contribute/products", openBase: "/resources/products", select: "id, product_name, manufacturer, price, quantity_in_lab, reorder_level, status, is_published, updated_at, materials(name), vendors(name)" },
  ...
  equipment: { table: "equipment", title: "Equipment", titleField: "name", newHref: "/contribute/equipment/new", editBase: "/contribute/equipment", openBase: "/resources/equipment", select: "id, name, manufacturer, model, equipment_type, status, is_published, updated_at" },
...
export function AtlasAdminPage() {
  ...
  async function addCategory(event: FormEvent<HTMLFormElement>) {
    ...
    const supabase = createClient();
    const { error } = await supabase.from("atlas_categories").insert({ code: String(form.get("code") || ""), title, slug: String(form.get("slug") || slugify(title)), description: String(form.get("description") || ""), sort_order: Number(form.get("sort_order") || 0), is_published: form.get("is_published") === "on" });
    if (error) setMessage(error.message); else { event.currentTarget.reset(); await load(); }
  }
  ...
  if (!isAdmin) return <section className="page-shell narrow-shell"><div className="notice">{message}</div></section>;
  return <section className="page-shell"><div className="page-heading split-heading"><div><p className="eyebrow">Administration</p><h1>Atlas</h1><p>Manage categories, entry publication, and public ordering for the editable Atlas.</p></div><Link className="button primary" href="/contribute/atlas/new">+ Add Entry</Link></div><form className="form-panel" onSubmit={addCategory}><div className="form-grid form-grid-3"><label>Code<input name="code" /></label><label>Title<input name="title" required /></label><label>Slug<input name="slug" /></label><label>Sort order<input name="sort_order" type="number" defaultValue="0" /></label><label><input name="is_published" type="checkbox" defaultChecked /> Published</label><label className="full">Description<textarea name="description" rows={2} /></label></div><button className="button" type="submit">+ Add Category</button></form>{message && <div className="notice">{message}</div>}<section className="detail-section"><p className="section-index">Categories</p><div className="admin-table">{categories.map((category) => <div className="library-admin-row" key={String(category.id)}><input defaultValue={String(category.code ?? "")} onBlur={(event) => updateCategory(String(category.id), { code: event.target.value })} /><input defaultValue={String(category.title ?? "")} onBlur={(event) => updateCategory(String(category.id), { title: event.target.value, slug: slugify(event.target.value) })} /><input type="number" defaultValue={String(category.sort_order ?? 0)} onBlur={(event) => updateCategory(String(category.id), { sort_order: Number(event.target.value) })} /><button type="button" onClick={() => updateCategory(String(category.id), { is_published: category.is_published === false })}>{category.is_published === false ? "Publish Category" : "Hide Category"}</button></div>)}</div></section><section className="detail-section"><p className="section-index">Entries</p><div className="admin-table">{entries.map((entry) => <div className="library-admin-row" key={String(entry.id)}><span><strong>{String(entry.title ?? "Untitled")}</strong><small>{typeof entry.atlas_categories === "object" && entry.atlas_categories ? String(entry.atlas_categories.title ?? "") : ""}</small></span><span><StatusPill status={String(entry.status ?? "draft")} /></span><span className="inline-actions"><Link href={`/atlas/${entry.id}`}>Open</Link><Link href={`/contribute/atlas/${entry.id}/edit`}>Edit</Link><select value={String(entry.status ?? "draft")} onChange={(event) => updateEntry(String(entry.id), event.target.value as EditorialStatus)}>{editorialStatuses.map((status) => <option key={status}>{status}</option>)}</select></span></div>)}</div></section></section>;
}
